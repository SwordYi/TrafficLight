...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM  wParam, LPARAM  lParam)
{
	...
	case WM_SIZE:
		...
		CreateRectPoint();
		CreateTextPoint();
		DeleteObject(hPenRed); // 重建画笔前删除原有画笔
...
void CreateArrowPoint()
{
	...
	for (int i = 6; i < 9; i++)
	{
		arrow[i][0].y = cc[i].y;					arrow[i][0].x = cc[i].x - radius * 7 / 10;
		arrow[i][1].y = cc[i].y - radius * 7 / 10;	arrow[i][1].x = cc[i].x;
		arrow[i][2].y = cc[i].y;					arrow[i][2].x = cc[i].x + radius * 7 / 10;
		arrow[i][3].y = cc[i].y;					arrow[i][3].x = cc[i].x;
		arrow[i][4].y = cc[i].y + radius * 7 / 10;	arrow[i][4].x = cc[i].x;
	}
}
